...
def main(filepath):
    ...
    plot_centerline_and_fitted_curve(
        original_path, curve, curve, tangents, tangents, special_index=30
    )
    ...
    print(
        f"Curve z range: {curve_points_origin[:, 2].min():.2f} to {curve_points_origin[:, 2].max():.2f}"
    )
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python main_lcia.py <file_path>")
        sys.exit(1)
